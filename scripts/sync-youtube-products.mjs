...
const CHANNEL_URL = process.env.YOUTUBE_CHANNEL_URL || 'https://www.youtube.com/@tiendanovamovilperu';
...
async function fetchFeed(channelId){ const feed=await fetch(`https://www.youtube.com/feeds/videos.xml?channel_id=${channelId}`,{headers:{'user-agent':'Mozilla/5.0'},signal:AbortSignal.timeout(15000)}); if(feed.ok){ const xml=await feed.text(); const entries=[...xml.matchAll(/<entry>([\s\S]*?)<\/entry>/gi)].slice(0,MAX_ENTRIES).map((m)=>{const e=m[1],videoId=textBetween(e,'yt:videoId'); return {videoId,title:textBetween(e,'title'),published:textBetween(e,'published'),description:textBetween(e,'media:description'),videoUrl:`https://www.youtube.com/shorts/${videoId}`};}).filter((e)=>e.videoId); if(entries.length) return entries; } console.warn(`Feed RSS no disponible (${feed.status}); usando la página de Shorts.`); const shortsUrl=`${CHANNEL_URL.replace(/\/$/,'')}/shorts`; const page=await fetch(shortsUrl,{headers:{'user-agent':'Mozilla/5.0'},signal:AbortSignal.timeout(15000)}); if(!page.ok) throw new Error(`No se pudo abrir Shorts (${page.status})`); const html=await page.text(); const ids=[...new Set([...html.matchAll(/"videoId":"([A-Za-z0-9_-]{11})"/g)].map((m)=>m[1]))].slice(0,MAX_ENTRIES); const entries=await Promise.all(ids.map(fetchWatchEntry)); return entries.filter(Boolean); }
